...
short LeArquivo(char **labirinto, FILE *arq, TipoNess *ness,TipoMonstroDatabase *monstroDatabase,TipoTamanhoArquivo *tamanhoArquivo) {
  ...
  while(!feof(arq) && !ferror(arq)){
    ...
    tamanhoArquivo->qtdLinhas = atoi(&dadosIniciais[12]);
    tamanhoArquivo->qtdColunas = atoi(&dadosIniciais[13]);
